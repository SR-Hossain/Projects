...
int bs = 23, totalboard[500][500], fruitposx, fruitposy, h=1,stageunlocked[4], stagenumber=1;
// char border = 219, c, ch;
int stagepoint[6]={100,50,250,200,200,150} ;
char border = 219;
...
int stageh;
int targetpoint[6] = {INT_MAX, 60, 50, 50, 50};
...
void game(bool key){
    if(key){
        ...
        while(gameover == 0){
            gotoXY(-2, 20, "+");
            cout<<point;
            if(stageh)cout<<"/"<<targetpoint[stageh];
            snake(1);
            fruit(1);
            key_press();
            if(etg)return;
        ...
        if(stageh == 0 and point>highscore){
            storepoint = fopen("Snake_Game_Score.txt", "w");
            fprintf(storepoint, "%d", point);
            fclose(storepoint);
        }
        gotoXY(-2, 20);cout<<"                 ";
        brdr(0);
