...
#define cursor(wid, row) SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), (COORD){wid+wincol/2-bc/2, row+winrow/2-br/2})
...
void new_console_size(){
    ...
    wincol = (int)vejachokh.srWindow.Right - vejachokh.srWindow.Left + 1;
    winrow = (int)vejachokh.srWindow.Bottom - vejachokh.srWindow.Top + 1;
    if(precol-wincol or prerow-winrow){
        if(tw)draw_afresh();
        else system("clear");
    }
